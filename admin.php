...
if ($id == "login" || substr($url, -1) == "p") {
  ...
  if (isset($_POST['login'])) {
    ...
    $stmt= $conn->prepare("select id, username, hash, session from users where username = ?");
    $stmt->bind_param("s", $user);
    $stmt->execute();
    $result = $stmt->get_result();
    $stmt-> close();
    $fetch = mysqli_fetch_array($result);
    if ($fetch && $fetch['username'] == $user && password_verify($pass, $fetch['hash'])) {
      $_SESSION["MIKHMON"] = $user;
      $_SESSION["user_session"] = $fetch['session'];
...
} elseif (isadmin() &$id == "settings" && !empty($session) || $id == "settings" && !empty($router) && isadmin()) {
  include_once('./include/menu.php');
  ...
  if ($API->connect($iphost, $userhost, decrypt($passwdhost))){
    ...
    echo "<script>window.location='./?session=$session&edit=$edit'</script>";
  } else {
...
} elseif ($id == "generate" && isadmin()) {
  include_once('./include/menu.php');
  include_once('./include/generate_operators.php');
} elseif ($id == "logout") {
  include_once('./include/menu.php');
